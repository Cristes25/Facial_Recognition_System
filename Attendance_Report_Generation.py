...
def fetch_course_schedule(self, professor_id, course_code, date):
    """
    ...
    WHERE professor_id = %s AND course_code = %s AND day_name = %s
    """
    day_name = date.strftime('%A')  # Get the day name (e.g., Monday)
    self.connector.mycursor.execute(query, (professor_id, course_code, day_name))
...
def fetch_attendance_report(self, course_code, date):
    ...
    query = """
    SELECT 
        s.student_name, s.course_name, s.attendance_date, s.attendance_status, s.attendance_reason 
    FROM student_attendance_schedule s
    WHERE s.course_code = %s AND s.attendance_date = %s
    """
...
############################
    ...
    def mark_attendance(self, student_id, course_code, date):
